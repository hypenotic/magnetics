<?php 
	$steps = get_post_meta($post->ID,'_steps',true);

    if(count($steps) > 1) {
 ?>
<section id="steps">
    <h2>Workflow</h2>
    <ol>
        <!-- For loop cycle through Array -->
        <?php
            foreach($steps as $step) {
            // Get custom meta values    
            $title  = $step['_title'];
            $image  = $step['_image'];
            $text  	= $step['_text'];
        ?>      
        <li>
            <?php if ($image) { ?>
            <div class="enlarges">
                <button data-action="activateModal"></button>
                <figure class="modal">
                    <img src="<?php echo wp_get_attachment_url($image); ?>" />
                </figure>
                <?php if (is_single('1515')) { ?>
                    <div style="background-image:url(<?php echo wp_get_attachment_url($image); ?>); background-size:contain; background-position:center center; height: 250px; width: 100%; background-repeat:no-repeat;"></div>
                <?php } else { ?>
                    <img class="frame" src="<?php bloginfo('template_url') ?>/dist/images/mockup_outline_mask.png" />
                    <figure style="background-image:url(<?php echo wp_get_attachment_url($image); ?>); background-size:contain; background-position:center center;">
                        <img src="data:image/png;base64,"/>
                    </figure>
                <?php } ?>
            </div>
            <?php } ?>

            <section class="description">
            <h3><?php if ($title) { 
               echo $title;
            } ?></h3>

            <p><?php if ($text) { 
               echo $text;
            } ?></p>
            </section>
            <br style="clear:both" />
       </li>

<?php 
		} 
?>
	</ol>

</section>

<?php  
    } 
?>
